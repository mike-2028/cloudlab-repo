...
locals {
  # see https://github.com/GoogleCloudPlatform/cloud-foundation-fabric/tree/master/modules/project#service-identities-and-authoritative-iam
  _cloudservices_iam = {
    "roles/editor" = [
  ...
  # grant requestor the same privielges as the group
  _requestor = replace(replace("user:${var.labels.requestor}", "__", "@"), "_", ".")
  _requestor_iam = {
  ...
  iam = {
    for role in distinct(concat(
      keys(local._cloudservices_iam),
      keys(var.iam),
      keys(local._requestor_iam),
      keys(local._group_iam),
      keys(local._service_accounts_iam),
      keys(local._service_identities_iam),
    )) :
    role => concat(
      try(local._cloudservices_iam[role], []),
      try(var.iam[role], []),
      try(local._requestor_iam[role], []),
  ...
  services = distinct(concat(var.services, var.defaults.services, local._services))
...
module "billing-alert" {
  ...
  calendar_period           = local.billing_alert.calendar_period
  custom_period             = local.billing_alert.custom_period
  pubsub_topic              = local.billing_alert.pubsub_topic
  notify_default_recipients = local.billing_alert.notify_default_recipients
