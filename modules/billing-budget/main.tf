...
resource "google_billing_budget" "budget" {
  ...
  budget_filter {
    ...
    dynamic "custom_period" {
      for_each = var.custom_period != null ? [1] : []
      content {
        dynamic "start_date" {
          for_each = var.custom_period.start_date != null ? [1] : []
          content {
            year  = var.custom_period.start_date.year
            month = var.custom_period.start_date.month
            day   = var.custom_period.start_date.day
          }
        }
        dynamic "end_date" {
          for_each = var.custom_period.end_date != null ? [1] : []
          content {
            year  = var.custom_period.end_date.year
            month = var.custom_period.end_date.month
            day   = var.custom_period.end_date.day
          }
        }
      }
    }
  ...
  all_updates_rule {
    ...
    disable_default_iam_recipients = (length(local.notification_channels) > 0 && !can(var.notify_default_recipients)) ? true : false
    schema_version                 = "1.0"
  }
}
